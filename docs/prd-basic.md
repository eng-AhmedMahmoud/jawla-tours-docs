...
| BR-011 | تأكيدات الحجز لازم تتم عبر مزود إيميل المعاملات (SendGrid أو Mailgun) مع نسبة وصول >98%. | Must | العمليات |
...
| BR-020 | المنصة لازم تكون قابلة للنشر على **Google Cloud Platform (GCP)** الخاصة بالعميل ببصمة صغيرة (Cloud Run + Cloud SQL تير صغير) لإبقاء تكاليف التشغيل تحت EGP 25,000/شهر في الباقة الأساسية. | Must | المالية |
...
| OQ-007 | إيه متطلب data residency؟ PII العملاء المصريين - هل ممكن يقعد على **GCP** في region أوروبا (مثل `europe-west3` Frankfurt) أو الشرق الأوسط (مثل `me-central1`) أو محتاج مركز بيانات محلي؟ | القانوني + الهندسة | M1 |
...
| الاستضافة | **Google Cloud Platform (GCP) — مقدَّم من العميل** (Cloud Run للـ backend + Cloud Run/Cloud Storage للـ frontend الثابت) | حساب GCP مملوك للعميل، تكلفة الاستضافة الشهرية على عاتقه مباشرة |
| المدفوعات | PayTabs (بوابة MENA متعددة العملات) | متطلب السوق المحلي والإقليمي |
| الإيميل | SendGrid | الوصول، خيارات توجيه IP لمصر |
| المراقبة | Better Uptime, Sentry, Cloud Monitoring + Cloud Logging (GCP) أساسي | تكلفة محكومة |
...
| تخزين الملفات | Google Cloud Storage (GCS) — bucket على حساب العميل | تذاكر و إيصالات PDF؛ متكامل مع باقي بنية GCP |
