...
- ربط مباشر مع نظام Amadeus العالمي للبحث عن الرحلات الجوية والحصول على الأسعار اللحظية
- ربط مع بوابة الدفع PayTabs لقبول الدفع داخل الموقع بالكامل (متعدد العملات: EGP/USD/EUR/SAR/AED)
- نظام إرسال رسائل بريد إلكتروني تلقائية لتأكيدات الحجز وإرسال تذاكر الطيران
...
| 0.2     | 2026-05-28 | Platform Arch Group | FR, NFR, API    | إضافة متطلبات الـ flights بعد Amadeus PoC                  |
...
| AT        | Acceptance Test                                                                     |
| BCD       | Booking Confirmation Document (e-ticket)                                            |
...
- مسار حجز موجّه للمستهلك لـ **flights** يظهر من خلال موقع الـ Next.js التسويقي
  الموجود حالياً، مع إضافة modules حجز محمية بـ authentication.
- backend بـ NestJS بيوفر **حوالي 18 REST endpoint** للبحث والحجز والدفع والإشعارات.
- مزوّد flights واحد (**Amadeus Self-Service / Enterprise Test endpoints**).
- الدفع بالكارت عن طريق **PayTabs** (PSP واحد للـ MVP، multi-currency: EGP/USD/EUR/SAR/AED).
...
- PayTabs بيدعم multi-currency native (EGP/USD/EUR/SAR/AED) عند الـ checkout.
...
| R-02   | PayTabs API Reference (PayPage v3 + IPN)                                     |
...
### Module: Booking (FR-033 — FR-043)

| ID     | المتطلب                                                                                                  | Priority |
| ------ | -------------------------------------------------------------------------------------------------------- | -------- |
| FR-033 | لازم النظام يصدر `bookingId` من السيرفر (UUIDv7) قبل بدء الدفع.                                          | MUST     |
| FR-034 | لازم النظام يحتفظ بالحجز بـ status: `DRAFT`، `PENDING_PAYMENT`، `CONFIRMED`، `FAILED`، `CANCELLED`، `REFUNDED`. | MUST     |
| FR-035 | لازم النظام يعتبر endpoint الحجز idempotent باستخدام `Idempotency-Key` header.                            | MUST     |
| FR-036 | لازم النظام ينهي الـ bookings اللي بحالة `DRAFT` بعد 15 دقيقة.                                            | MUST     |
| FR-037 | لازم النظام ينهي الـ bookings اللي بحالة `PENDING_PAYMENT` بعد 30 دقيقة (PayTabs transaction auto-expire متوافق).   | MUST     |
| FR-038 | لازم النظام يسمح للمستخدم يشوف كل حجوزاته مقسّمة على صفحات بترتيب تنازلي بالتاريخ.                         | MUST     |
| FR-039 | لازم النظام يسمح للمستخدم بإلغاء حجز refundable قبل deadline المزوّد.                                     | MUST     |
| FR-040 | لازم النظام يضيف audit-log row غير قابل للتعديل لكل تغيير في الـ status.                                   | MUST     |
| FR-041 | لازم النظام يعمل rollback لحجز المزوّد لو إصدار الـ ticket النهائي فشل.                                    | MUST     |
| FR-042 | لازم النظام ينتج `bookingReference` (8 خانات alphanumeric، Crockford base32) معروض للمستخدم.              | MUST     |
| FR-043 | لازم النظام يصدر domain events (`BookingConfirmed`, `BookingCancelled`) على BullMQ للمستهلكين.            | MUST     |

### Module: Payment (FR-044 — FR-052)

| ID     | المتطلب                                                                                                              | Priority |
| ------ | -------------------------------------------------------------------------------------------------------------------- | -------- |
| FR-044 | لازم النظام ينشئ PayTabs transaction بـ `auth` mode (pre-authorization) عند الـ checkout.                              | MUST     |
| FR-045 | لازم النظام يفرض 3DS2 challenge (PayTabs `tran_class=ecom` مع 3DS مفعّل).                                              | MUST     |
| FR-046 | لازم النظام يعمل capture للـ PayTabs transaction بعد ما إصدار الـ ticket من المزوّد ينجح.                              | MUST     |
| FR-047 | لازم النظام يعمل reconcile عن طريق PayTabs IPN webhooks (`tran_type=Sale`, `Auth`, `Capture`, `Refund`, `Void`).        | MUST     |
| FR-048 | لازم النظام يتحقق من signatures الـ PayTabs IPN بالـ server-key + HMAC-SHA256.                                          | MUST     |
| FR-049 | لازم النظام يتحمل تكرار الـ webhook (idempotent على `tran_ref`).                                                        | MUST     |
| FR-050 | لازم النظام يعرض payment-error code mapping يتحوّل لرسالة user-friendly localized.                                     | MUST     |
| FR-051 | في Basic، ممكن الـ refunds تتعمل يدوي من Ops عن طريق PayTabs merchant dashboard؛ والنظام لازم يعمل reconcile عبر webhook. | MAY  |
| FR-052 | لازم النظام **مايخزّنش** PAN أو CVV أو بيانات الـ track كاملة — PayTabs hosted payment page (PayPage) tokenization بس. | MUST     |

### Module: Notifications (FR-053 — FR-058)

| ID     | المتطلب                                                                                                      | Priority |
| ------ | ------------------------------------------------------------------------------------------------------------ | -------- |
| FR-053 | لازم النظام يبعت email transactional عند: تأكيد الحجز، فشل الدفع، الإلغاء، الـ refund.                        | MUST     |
| FR-054 | لازم النظام يحط كل الإشعارات الخارجة في طابور على BullMQ (`notifications.email` queue).                       | MUST     |
| FR-055 | لازم النظام يعيد محاولة إرسال الـ email حتى 5 مرات بـ exponential backoff (base 30 ثانية، factor 2).         | MUST     |
| FR-056 | لازم النظام يـ localize الـ emails لـ AR (الافتراضي لمصر)، EN، FR (الـ locale يتحدد من profile المستخدم).      | MUST     |
| FR-057 | لازم النظام يمنع الإشعارات المكررة (de-dup key = `${eventId}:${userId}:${template}`).                          | MUST     |
| FR-058 | لازم النظام يحترم تفضيلات الإشعارات للمستخدم (الـ transactional emails مش ممكن يعمل opt-out منها).             | MUST     |

### Module: Admin (FR-059 — FR-066)

| ID     | المتطلب                                                                                                     | Priority |
| ------ | ----------------------------------------------------------------------------------------------------------- | -------- |
| FR-059 | لازم الـ admin console يسمح بالبحث في الـ bookings بـ reference، email العميل، supplier ref، status، التاريخ. | MUST     |
| FR-060 | لازم الـ admin console يسمح بعرض تفاصيل الحجز كاملة مع الـ audit log.                                         | MUST     |
| FR-061 | لازم الـ admin console يسمح للموظفين بإعادة إرسال notification.                                              | MUST     |
| FR-062 | لازم الـ admin console يسمح للموظفين بتعليم الحجز كـ refunded بعد refund يدوي من PayTabs (reconciliation).    | MUST     |
| FR-063 | لازم الـ admin console يسمح للموظفين بعرض المستخدمين (بحث بالـ email، عرض profile أساسي، آخر login).         | MUST     |
| FR-064 | لازم الـ admin console يكون محمي بـ role `admin` ويطلب re-authentication كل 12 ساعة.                          | MUST     |
| FR-065 | لازم الـ admin console يـ log كل فعل من الموظف بالـ actor والـ target والفرق before/after.                    | MUST     |
| FR-066 | المفروض الـ admin console يعرض tile KPIs يومي (الحجوزات النهارده، GMV النهارده، نسبة الفشل).                  | SHOULD   |
...
| NFR-002 | كل الـ endpoints اللي مش بتاعت مزوّد لازم تكون p95 ≤ 250 ميلي ثانية.                                     |
| NFR-003 | Largest Contentful Paint (LCP) لصفحة الـ home ≤ 2.0 ثانية على 4G profile.                                 |
| NFR-004 | Time-to-Interactive (TTI) لصفحة نتائج البحث ≤ 3.5 ثانية.                                                  |
| NFR-005 | هدف نسبة الـ search cache hit ≥ 35% خلال ساعات الشغل.                                                     |

### القابلية للتوسع (Scalability)

| ID      | المتطلب                                                                                                  |
| ------- | -------------------------------------------------------------------------------------------------------- |
| NFR-006 | لازم النظام يتحمّل 50 RPS booking-funnel sessions مستمرة لمدة ساعة.                                       |
| NFR-007 | لازم النظام يتحمّل 300 RPS على endpoints الـ catalog للقراءة فقط مع horizontal scale.                     |
| NFR-008 | لازم الـ BullMQ workers تتقدر تتوسع أفقياً لـ 4 replicas من غير تعديل في الكود.                            |
| NFR-009 | لازم اتصالات الـ Database تكون pooled عن طريق PgBouncer (transaction mode، حد أقصى 200 client conns).     |

### التوافر (Availability)

| ID      | المتطلب                                                                                                  |
| ------- | -------------------------------------------------------------------------------------------------------- |
| NFR-010 | الـ uptime الشهري المستهدف 99.5% (downtime ≤ 3 ساعات و39 دقيقة).                                          |
| NFR-011 | RPO ≤ 15 دقيقة، RTO ≤ 4 ساعات للـ DR الكامل في الـ primary region.                                        |
| NFR-012 | لازم Postgres ينشر مع synchronous standby replica واحد.                                                   |
| NFR-013 | لازم Redis ينشر مع replica واحد + Sentinel.                                                                |
| NFR-014 | لازم نوافذ الصيانة المخططة تتعلن قبلها بـ 72 ساعة على الأقل.                                              |
...
| ID      | المتطلب                                                                                                  |
| ------- | -------------------------------------------------------------------------------------------------------- |
| NFR-020 | لازم النظام يدعم AR (RTL)، EN، FR لكل الواجهات المعروضة للمستخدم.                                         |
| NFR-021 | لازم كل التواريخ تتعرض بـ timezone اللي المستخدم اختاره؛ والتخزين لازم يكون UTC.                          |
| NFR-022 | لازم كل القيم المالية تتضمن صراحة ISO 4217 currency codes.                                                |

### إمكانية الوصول (Accessibility)

| ID      | المتطلب                                                                                                  |
| ------- | -------------------------------------------------------------------------------------------------------- |
| NFR-023 | لازم الموقع يتوافق مع WCAG 2.1 AA (semantic landmarks، focus order، color contrast ≥ 4.5:1).             |
| NFR-024 | لازم كل العناصر التفاعلية تكون قابلة للتشغيل بالكيبورد؛ ومؤشرات focus ظاهرة مطلوبة.                       |
| NFR-025 | لازم الـ forms تعرض رسايل الخطأ بطريقة programmatic عن طريق `aria-describedby`.                          |

### الامتثال (Compliance)

| ID      | المتطلب                                                                                                  |
| ------- | -------------------------------------------------------------------------------------------------------- |
| NFR-026 | PCI scope: SAQ A-EP. مفيش PAN يلمس السيرفرات بتاعتنا.                                                    |
| NFR-027 | GDPR: مسارات data-export وdata-deletion لازم تكون متاحة خلال 30 يوم من الطلب.                            |
| NFR-028 | Cookie consent: opt-in صريح للكوكيز اللي مش أساسية؛ مع تحكم granular.                                    |
...
| `booking_items`           | عناصر لكل منتج (flight)                                               | N—1 → `bookings`                                |
| `flight_segments`         | segment لكل leg من رحلة flight item                                   | N—1 → `booking_items`                           |
| `passengers`              | تفاصيل الراكب المرتبطة بالحجز                                          | N—1 → `bookings`                                |
| `payments`                | سجلات lifecycle الـ PaymentIntent                                     | N—1 → `bookings`                                |
| `payment_webhook_events`  | مخزن de-duplication لـ events الـ PayTabs                             | —                                               |
...
CREATE TABLE payments (
  ...
  status              TEXT NOT NULL
                      CHECK (status IN ('requires_action','authorized',
...
| 14 | POST   | `/bookings`                                | bearer      | `{type,offerId,passengers[],contact}` + Idempotency  | `{bookingId,reference,status}`        | 201,400,409,422     |
| 15 | GET    | `/bookings/{id}`                           | bearer      | —                                                    | `Booking`                             | 200,403,404         |
| 16 | GET    | `/bookings?limit=&cursor=`                 | bearer      | paging                                               | `{items[],nextCursor}`                | 200                 |
| 17 | POST   | `/bookings/{id}/cancel`                    | bearer+OTP  | `{reason,otp}`                                       | `{status}`                            | 200,403,409         |
| 18 | POST   | `/payments/paytabs`                        | bearer      | `{bookingId}` + Idempotency                          | `{paymentUrl,tranRef,amount}`         | 201,409             |
| 19 | POST   | `/payments/webhooks/paytabs`               | sig         | PayTabs IPN event                                    | `200`                                 | 200,400             |
| 20 | GET    | `/admin/bookings?...`                      | admin       | filters                                              | `{items[],nextCursor}`                | 200,403             |
| 21 | POST   | `/admin/bookings/{id}/refund-reconcile`    | admin       | `{paytabsRefundRef,amount}`                          | `{status:'REFUNDED'}`                 | 200,403,409         |
| 22 | POST   | `/admin/notifications/{id}/resend`         | admin       | —                                                    | `{queued:true}`                       | 202,403,404         |
...
| A10 SSRF                          | outbound allow-list لـ Amadeus/PayTabs؛ مع mitigation لـ DNS rebinding؛ مفيش URLs من المستخدم.   |
...
- SAQ **A-EP** — حقول الدفع بتترندر بواسطة PayTabs PayPage (hosted)؛ الـ backend
...
2. السيرفر بيتحقق من الـ offer (`flights/price-check`).
3. السيرفر بيدخل row في `bookings` (`status=DRAFT`، `expires_at=now()+15m`).
4. الـ Client بينادي `POST /payments/paytabs` → بيتنشئ PayTabs transaction
   (`tran_type=Auth`)، الحجز يبقى → `PENDING_PAYMENT`، `expires_at=now()+30m`.
5. الـ Client بيتحوّل لـ PayTabs PayPage (3DS) لإدخال بيانات البطاقة.
6. PayTabs IPN `tran_type=Auth, response_status=A` بيوصل.
7. الـ Worker `booking.fulfill` بيشتغل: بينادي Amadeus لإصدار PNR.
8. لو نجح، بيتنادى PayTabs `Capture` → الحجز → `CONFIRMED`،
   ويتعمل enqueue للـ notification.
9. لو فشل المزوّد: PayTabs `Void` (void uncaptured authorization)، الحجز → `FAILED`،
...
| PayTabs transaction create    | 6 ثواني     | retry مرة واحدة، بعدها 503.                                                    |
| PayTabs confirm (client)      | 30 دقيقة    | الحجز يتعمل له FAIL تلقائي، والـ transaction يتلغي.                            |
| Amadeus PNR creation          | 30 ثانية    | حتى 3 retries؛ في النهاية لو فشل يعمل Void للـ transaction ويـ FAIL الحجز.    |
| PayTabs capture               | 10 ثواني    | retry حتى 5 مرات؛ في النهاية لو فشل يـ page on-call، ويبدأ supplier rollback. |
...
Client                       Next.js                   NestJS               Redis           Amadeus
  ...
  │                          │                           │◀────────────────────────────────────│
  │ checkout (pax + pay)     │                           │                     │              │
  │─────────────────────────▶│ POST /bookings + /payments/paytabs              │              │
  │                          │                           │                     │              │
  │ 3DS challenge            │                           │                     │              │
  │ PayTabs → authorized     │ IPN                       │                     │              │
  │                          │                           │ worker.fulfill:     │              │
...
- `TICKETING_FAILED`: void PayTabs (uncaptured authorization)؛ compensating call للمزوّد لو الـ PNR
...
│  Client      │─────────────────────▶│ NestJS API  │
└─────────────┘                       └────┬────────┘
       │                                    │  PayTabs.payment.request (tran_type=Auth)
       │                                    ▼
       │                              ┌──────────┐
       │  payment_url                 │ PayTabs  │
       │◀─────────────────────────────└──────────┘
       │ redirect to PayPage (hosted, 3DS)
       │                                    
       ▼                                    
   3DS Challenge (issuer)                   
       │                                    
       ▼                                    
   tran_type=Auth, response_status=A (authorized)
       │                                    
       ▼                                    
   PayTabs IPN ──────────────▶ NestJS verify sig ─▶ enqueue booking.fulfill
                                                          │
                                                          ▼
                                                    supplier PNR/booking
                                                          │ success
                                                          ▼
                                                    PayTabs.capture(tran_ref)
                                                          │
                                                          ▼
...
3. فريق الـ Ops بيتنبه؛ الـ refund بيتعمل من PayTabs merchant dashboard.
4. IPN `tran_type=Refund` من PayTabs بيوصل → بيتحدّث `payments.refunded_amount_minor`،
...
| تأخر PayTabs IPN > 5 دقايق                                           | page     | PagerDuty    |
...
| Contract     | Pact (Amadeus, PayTabs stubs)          | كل تكاملات المزوّدين      |
...
| AT-013  | دفع بـ 3DS مطلوب                     | كارت اختبار بيطلب 3DS2                                              | بأكد الـ transaction                            | challenge المُصدر بيظهر؛ لو نجح، الـ PayTabs transaction بيتحول لـ `authorized`.       |
| AT-014  | signature الـ webhook                | IPN بـ PayTabs signature غير صالح وصل                              | NestJS بيتحقق                                    | 400 `INVALID_SIGNATURE`؛ مفيش تغيير في الـ state.                                     |
| AT-015  | rollback فشل ticketing من المزوّد    | إنشاء PNR في Amadeus بيرجع 5xx 3 مرات                              | الـ Worker استنفذ الـ retries                    | PayTabs Authorization اتعملها Void؛ الحجز FAILED؛ email "مفيش خصم" اتبعت للعميل.       |
| AT-016  | reconcile للـ refund اليدوي          | فريق Ops عمل refund من PayTabs dashboard                            | IPN `tran_type=Refund` وصل                       | `payments.refunded_amount_minor` اتحدّث؛ الحجز REFUNDED لو كان كامل.                  |
| AT-017  | الإلغاء بيطلب OTP                    | أنا صاحب الحجز                                                       | بنادي `/bookings/{id}/cancel` من غير OTP         | 401 `OTP_REQUIRED`.                                                                    |
| AT-018  | منع قراءة حجز عبر الـ tenants        | أنا user A                                                          | بعمل GET `/bookings/{idOfUserB}`                 | 403 Forbidden.                                                                         |
| AT-019  | audit log للـ admin                  | أنا admin وبعمل إعادة إرسال notification                            | الفعل اكتمل                                       | row في `audit_events` اتدخل بـ actor، target، before/after.                            |
| AT-020  | rate limit للـ auth                  | أصدرت 11 محاولة login في 60 ثانية من IP واحد                        | المحاولة الـ 11                                  | 429 مع `Retry-After`.                                                                  |
| AT-021  | التعريب (AR)                         | الـ locale في profile = `ar`                                        | بستلم email تأكيد حجز                            | الـ Email بيظهر RTL بنص AR.                                                            |
| AT-022  | WCAG focus order                     | بعمل tab في form البحث                                              | بلاحظ الـ focus                                  | الـ Focus بيزور الـ inputs بترتيب DOM؛ مع focus ring ظاهر يحقق contrast 3:1.          |
| AT-023  | Backup PITR                          | أنا DBA                                                              | شغّلت PITR لـ 30 دقيقة فاتت                       | الـ Restore بيكتمل في < 4 ساعات (RTO)؛ فقد البيانات ≤ 15 دقيقة (RPO).                  |
...
| R-02 | تساقط المستخدمين عند 3DS challenge في PayTabs               | متوسط      | متوسط  | تذكير بالسعر قبل 3DS، نص "save card" بـ A/B.              |
