...
- AC-087.1: سجل تدقيق append-only لتخزين ثابت (GCS bucket مع Bucket Lock / Object Versioning، أو Spanner مع change streams، أو ما يعادله على GCP).
...
| R-055 | تعقيد تشغيل Kubernetes يتجاوز قدرة الفريق | عالٍ | عالٍ | استخدم K8s مُدار (**GKE على GCP**) أو **Cloud Run** كبديل أبسط؛ توثيق كل شيء؛ دوران on-call؛ تدريب SRE | Engineering Leadership |
...
| OQ-072 | مشغّل Kubernetes — **GKE** المُدار على GCP (افتراضي للمنصة بأكملها) أم نستخدم **Cloud Run** كبديل بدون-سيرفر داخل نفس GCP؟ | Engineering | M1 |
...
|--------|---------------------------|
| Compute | Kubernetes (**GKE على Google Cloud Platform — مقدَّمة من العميل**)، autoscaling، multi-zone، multi-region |
| Web (Responsive) | Next.js (أو ما يعادله) مع تصميم متجاوب يخدم الكمبيوتر، التابلت، والموبايل من نفس الكود؛ PWA اختياري |
| API Platform | API Gateway (Kong، **GCP API Gateway / Apigee**، أو داخلي)؛ بوابة مطورين |
